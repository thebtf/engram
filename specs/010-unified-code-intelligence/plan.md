...
**Storage**: PostgreSQL 17 plus pgvector is the authoritative state store. New `ci_*` tables hold rebuildable code-derived facts. `uci_exposures` and `uci_completion_evidence` hold durable append-only non-content UCI evidence under the UCI `ExposureRecorder` owner. A daemon-local SQLite registry persists only approved roots, local checkout evidence, dirty paths, watcher configuration, and resume checkpoints; it is never a query, graph, exposure, or authorization authority.
...
**Constraints**: Resolve and authorize Source/Checkout/View before candidate selection; record each authorized search, graph result, and versioned read only after that authorization and a closed result decision; return a result body only after its initial evidence append succeeds; publish views atomically under a fenced lease; never mix view generations; preserve project/Space semantics; expose `stale`, `partial`, `offline`, `unsupported`, and vector degradation honestly as retrieval state/coverage, while completion is `unknown` absent qualifying evidence or a verified supported-host `succeeded`/`partial`/`failed`/`abandoned` outcome; preserve existing authorization; keep credentials and excluded source bodies out of artifacts, embeddings, evidence records, logs, and responses; and do not turn a path, branch, remote, label, or legacy key into authority.
...
| Automatic retrieval exposure and completion | PASS | The UCI-owned `ExposureRecorder` writes durable, append-only, scoped non-content `uci_exposures` evidence from the authorized closed MCP search, graph, or read decision. A verified supported-host callback may append closed `succeeded`, `partial`, `failed`, or `abandoned` `uci_completion_evidence`; no qualifying callback remains `unknown`. Canonical binding digests return an original record only for an exact retry and reject a mismatch with non-disclosing `IDEMPOTENCY_MISMATCH`. An initial append failure suppresses the result body and returns `EXPOSURE_UNAVAILABLE`; a completion append failure returns `COMPLETION_EVIDENCE_UNAVAILABLE` only to the callback. The evidence grants no access, View, publication, or product-success authority. |
...
`research.md`, `data-model.md`, and `quickstart.md` preserve the pre-design gate: the execution order resolves authority before index data and exposure recording, fences publication before graph/search, profiles vectors, makes async recovery durable, keeps legacy data readable but non-authoritative for UCI, and leaves UCI-2/3 outside the implementation boundary.
...
| Server-authorized Source/Checkout/View registry; PostgreSQL `ci_*` code projections plus UCI-owned durable non-content `uci_*` evidence; daemon-local SQLite operational state; existing daemon/server transport | Selected | It matches the constitution and adopted contracts, isolates dirty worktrees, records authorized retrieval without promoting a second authority, supports atomic publication and durable recovery, and does not add a network service. |
...
All Phase 0 decisions and their source evidence are consolidated in [research.md](research.md). In particular, it resolves migration numbering, current UCI seams, daemon/client context ownership, PostgreSQL/SQLite roles, UCI-owned exposure/completion ownership, parser licensing/build boundary, native Windows worktree testing, protobuf ownership, real-vector requirements, and watcher/recovery posture.
...
- `contracts/query-response.schema.json` and `contracts/examples.json` define the closed public response shape, including the bounded opaque exposure receipt.
...
| 2 | Code + Test | Add `ci_*` code-projection storage and fenced publication via additive migration `172_uci_index_projection`: staging, manifest finalization, generation intervals, jobs, lease epoch, and durable append-only `uci_exposures`/`uci_completion_evidence` with canonical binding digests. | Slice 1 supplies authorized binding. | Partial upload, lost ACK replay, stale lease, delete-all-vs-failed-scan, exact evidence retry, evidence mismatch, and recorder-failure scenarios leave current views and evidence records coherent. |
| 3 | Code + Test | Replace line-block UCI processing with Go AST extraction, shared artifact facts, FTS, and bounded graph resolution over the same published View. This slice also ships the adopted structured/text adapters: Markdown headings/links, JSON/YAML keys and local references, SQL DDL as text only, and OpenAPI paths/operations/local references without external-ref fetches. Migrate every UCI caller from legacy `code_chunks` semantics; retain old rows only as explicit `legacy_unscoped` compatibility/rollback data. | Slice 2 supplies atomic View membership. | Exact/FTS/explain/impact results cite one View. Supported structured/text facts are View-pinned and report coverage. SQL never executes, and OpenAPI never fetches external refs. Malformed Go is fresh and partial, not silently stale. |
| 4 | Code + Test | Cut native MCP context isolation through the existing daemon, `EngramService`, gRPC server, and MCP tool surfaces. Introduce the Windows-safe disposable installation and standard-MCP-client harness. Preserve tool names; add `codebase_context`, `codebase_graph`, and `codebase_read` only as adopted APIs. At the shared MCP boundary, record every authorized search, graph result, and versioned read through `ExposureRecorder`; only a verified supported-host callback may append a closed completion outcome. Append protobuf fields/RPCs; do not reuse existing tags or add a second service. | Slices 1–3 establish typed core state. | The harness installs the built daemon/server in a disposable native target and drives standard MCP clients. It proves isolated contexts, evidence receipts, `EXPOSURE_UNAVAILABLE` result suppression, exact retries, mismatches, and no-callback `unknown` without a direct service-call substitute. |
...
| 8 | Review + Acceptance | Build and exercise UCI-1A+B through the versioned Slice 4 Windows-safe disposable installation and standard-MCP-client harness: server/daemon/parser worker, two standard MCP clients, real PostgreSQL, real provider, authorization-negative matrix, durable exposure/completion evidence, recorder health/failure behavior, 12-task baseline, SLO samples, exact-head regression/SonarQube, and rollback receipt. | Slices 0–7 and the completed Slice 4 harness. | All UCI-1 assigned scenarios, SC-01 through SC-08, are evidenced through the installed path without claiming UCI-2/3. |

## Migration and Cutover Plan

1. **Expand** — Verify the current migration registry at implementation start. This plan binds the observed high-water mark to migration 170; reserve `171_uci_context_registry` and `172_uci_index_projection` only if no intervening approved migration is present. Both are additive, idempotent forward migrations. Migration 172 includes `uci_exposures` and `uci_completion_evidence` as durable evidence tables and `ci_*` tables as code projections.
...
3. **Rebuild derived code truth** — Existing `code_chunks`/embeddings remain `legacy_unscoped`; do not backfill a checkout, commit, View, or provenance guessed from paths, labels, or hashes. Build UCI code projections from allowed saved source bytes. Do not rebuild UCI evidence from source data.
4. **Cut callers together** — Move daemon `codebase_index/status/search` and new read/graph/context calls to the typed workflow in one controlled wave. The shared MCP query/read/graph boundary records evidence only after authorization and response result construction. It returns a body only after initial evidence append success. A legacy project selector can enter only the compatibility resolver; it cannot select a dirty checkout, global latest view, or neighboring tree.
...
6. **Rollback** — Before cutover, disable the new UCI route, leave `ci_*` code projections inert, and leave UCI evidence tables empty because no authorized boundary is writing them; product-domain data and legacy indexes remain untouched. After expand/cutover but before contraction, route back through the prior compatibility path while preserving added schema/history. Code projections may be rebuilt from authorized source after a future contraction. UCI evidence restores only through normal PostgreSQL backup/restore plus integrity verification. No rollback changes crypto-bound identity data or returns secrets to local components.

## Single Release Map

| Release | Closes | Included implementation checkpoints | Required release evidence | Rollback boundary |
|---|---|---|---|---|
| UCI-1 (UCI-1A+B together) | Agents could not safely search/trace exact saved code in two concurrent worktrees; they can now use one installed daemon without cross-worktree context leakage. | 0–8 above | Real PostgreSQL, the versioned Slice 4 Windows-safe disposable installation and standard-MCP-client harness, built native server/daemon/parser worker, two-client installed Windows proof, structured/text coverage, real semantic provider, authorization, durable non-content exposure with no-callback `unknown`, verified supported-host `partial` completion, exact/mismatch idempotency, recorder health/failure, recovery evidence, 12-task measurement, and exact-head regression/SonarQube. | New route can be disabled before contraction; `ci_*` code projections are rebuildable and legacy data stays explicitly legacy until the UCI-3 retirement gate. Durable UCI evidence is preserved through its normal PostgreSQL backup/restore and integrity-verification path. |
...
internal/mcp/                           # server MCP schemas and handlers; remove raw project authority and record evidence only after UCI authorization
internal/grpcserver/                    # existing EngramService implementation; add private UCI RPCs
internal/db/gorm/                       # migration registry, PostgreSQL models/stores, UCI evidence tables, current legacy code_chunks
...
| FR-04, FR-05, FR-06, FR-08, FR-10 | 2–4 | planned UCI domain package, including the UCI-owned `ExposureRecorder`; `internal/db/gorm`; `internal/mcp`; `internal/grpcserver`; replace `internal/retrieval/code_hybrid.go` raw-project path |
...
| No generic retrieval exposure/completion recorder exists in the current product. | Add the UCI-owned `ExposureRecorder` and durable append-only `uci_exposures`/`uci_completion_evidence` evidence. Bind retries canonically, preserve evidence through PostgreSQL backup/restore with integrity checks, expose scoped recorder health, and never infer completion from a response or let evidence grant authority. |
