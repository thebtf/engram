...
After Source, Checkout, and View authorization, every code search, graph result, and versioned read records one idempotent non-content UCI exposure. A verified supported-host callback may append only `succeeded`, `partial`, `failed`, or `abandoned` completion evidence; without qualifying evidence completion remains `unknown`, independently of retrieval result state and coverage.
...
**Storage**: PostgreSQL 17 plus pgvector is the authoritative state and projection store. New `ci_*` tables hold code-derived facts and non-content retrieval exposure/completion projections. A daemon-local SQLite registry persists only approved roots, local checkout evidence, dirty paths, watcher configuration, and resume checkpoints; it is never a query, graph, exposure, or authorization authority.
...
**Constraints**: Resolve and authorize Source/Checkout/View before candidate selection; record each authorized search, graph result, and versioned read only after that authorization; publish views atomically under a fenced lease; never mix view generations; preserve project/Space semantics; expose `stale`, `partial`, `offline`, `unsupported`, and vector degradation honestly as retrieval state/coverage, while completion is `unknown` absent qualifying evidence or a verified supported-host `succeeded`/`partial`/`failed`/`abandoned` outcome; preserve existing authorization; keep credentials and excluded source bodies out of artifacts, embeddings, exposure records, logs, and responses; and do not turn a path, branch, remote, label, or legacy key into authority.
...
| Automatic retrieval exposure and completion | PASS | The UCI-owned `ExposureRecorder` writes an idempotent, scoped, non-content `ci_exposures` projection after Source/Checkout/View authorization. Only a verified supported-host callback can append closed `succeeded`, `partial`, `failed`, or `abandoned` completion evidence; no qualifying callback remains `unknown`. Completion does not restate retrieval result state or coverage, and the projection grants no authority. |
...
| 2 | Code + Test | Add `ci_*` projection storage and fenced publication via additive migration `172_uci_index_projection`: staging, manifest finalization, generation intervals, jobs, lease epoch, idempotent `ci_exposures`, and optional `ci_completion_evidence`. | Slice 1 supplies authorized binding. | Partial upload, lost ACK replay, stale lease, delete-all-vs-failed-scan, and exposure retry scenarios leave current views and exposure references coherent. |
...
| 4 | Code + Test | Cut native MCP context isolation through the existing daemon, `EngramService`, gRPC server, and MCP tool surfaces. Introduce the Windows-safe disposable installation and standard-MCP-client harness. Preserve tool names; add `codebase_context`, `codebase_graph`, and `codebase_read` only as adopted APIs. At the shared MCP boundary, record every authorized search, graph result, and versioned read through `ExposureRecorder`; only a verified supported-host callback may append a closed completion outcome. Append protobuf fields/RPCs; do not reuse existing tags or add a second service. | Slices 1–3 establish typed core state. | The harness installs the built daemon/server in a disposable native target and drives standard MCP clients; it proves a verified `partial` completion callback separately from a no-callback `unknown` receipt. |
...
internal/mcp/                           # server MCP schemas and handlers; remove raw project authority and record exposure only after UCI authorization
internal/grpcserver/                    # existing EngramService implementation; add private UCI RPCs
internal/db/gorm/                       # migration registry, PostgreSQL models/stores, UCI exposure projection, current legacy code_chunks
...
| FR-04, FR-05, FR-06, FR-08, FR-10 | 2–4 | planned UCI domain package, including `ExposureRecorder`; `internal/db/gorm`; `internal/mcp`; `internal/grpcserver`; replace `internal/retrieval/code_hybrid.go` raw-project path |
...
| No generic retrieval exposure/completion recorder exists in the current product. | Add the UCI-owned `ExposureRecorder` and additive `ci_exposures`/`ci_completion_evidence` projection. Do not reuse product-specific records, infer completion from a response, or let the projection grant authority. |
