...
6. Force initial exposure-recorder unavailability and verify `EXPOSURE_UNAVAILABLE`, `status: unavailable`, `exposure: null`, and no result body/items. Retry the same binding and then a changed binding; only the exact retry returns its original receipt. Force a completion write failure and verify that only the callback receives `COMPLETION_EVIDENCE_UNAVAILABLE`, without a changed exposure or manufactured completion.
