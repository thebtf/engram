...
5. A completion callback is optional only where the installed host does not support it. The no-callback case is a required explicit `unknown` assertion. A verified supported-host callback must also prove a `partial` completion outcome without treating retrieval result state or coverage as host completion. The evidence gates also cover initial exposure-recorder unavailability, exact retry, exposure/completion idempotency mismatch, callback write failure, and scoped secret-free recorder health.
...
| Authorization, exposure, and completion | `go test ./tests/uci/acceptance -run '^TestUCIAuthorizationMatrix$'` | Every authorized search, graph result, and versioned read records one durable opaque exposure after context authorization. A closed authorized unavailable result retains the contextual envelope, closed error, empty items, and receipt while recorder health stays healthy. Denials record nothing. Exact retry returns the original receipt only for the same canonical binding; a mismatch returns `IDEMPOTENCY_MISMATCH` without a stale receipt or contextual envelope. An unavailable initial append returns `EXPOSURE_UNAVAILABLE`, `status: unavailable`, `exposure: null`, and no contextual envelope/body/items. Unsupported/no-callback completion remains `unknown`; a verified supported host records `partial` independently of retrieval result state and coverage. Completion-write failure leaves the exposure unchanged and returns `COMPLETION_EVIDENCE_UNAVAILABLE` only to the callback. |
| Real vector | `go test ./internal/uci -run '^TestUCISemanticRealProviderConceptualHitMatchesScopedPostgresBaseline$' -count=1 -v` with `UCI_SEMANTIC_REAL_PROVIDER=1`, disposable `UCI_SEMANTIC_TEST_DSN`, and explicitly supplied provider/model/profile prerequisites | A real provider generates corpus and zero-lexical-overlap query vectors that agree with the exact scoped PostgreSQL baseline. `TestUCIRetrievalSlice` remains a deterministic mechanical integration proof and cannot satisfy this row. Ordinary fresh-index → durable jobs → production `EmbeddingWorker` → full-corpus vectors → MCP result is the separate T099 gate. |
...
4. Confirm that every returned result identifies a consistent Source/Checkout/View, and that A/B never observe the other dirty state. Confirm that each authorized search, graph result, and versioned read returns one opaque exposure ref only after the evidence append succeeds.
...
6. First force a closed authorized unavailable result while the recorder is healthy and verify its contextual envelope, closed source/index error, empty items, and opaque exposure receipt. Then force initial exposure-recorder unavailability and verify the distinct `EXPOSURE_UNAVAILABLE`, `status: unavailable`, `exposure: null`, and no contextual envelope, result body, or items. Retry the same binding and then a changed binding; only the exact retry returns its original receipt. Force a completion write failure and verify that only the callback receives `COMPLETION_EVIDENCE_UNAVAILABLE`, without a changed exposure or manufactured completion.
7. Repeat denied, revoked, mismatched, ambiguous, private-checkout, stale-view, pagination, and graph-hop cases. A refusal must disclose no outside body, identifier, count, relationship, local Windows path, or exposure reference.
...
- authorized search/graph/read durable evidence, including canonical exact retry, no-record denial, recorder-unavailable body suppression, non-disclosing idempotency mismatches, a verified supported-host `partial` completion callback, a completion-write failure that preserves its parent, and an explicit `unknown` no-callback case;
