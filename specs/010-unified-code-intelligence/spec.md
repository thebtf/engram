...
- An initial exposure append fails, a completion append fails, or an opaque idempotency key is reused with a different client, request, authorized context, closed result, verified callback, or outcome.
...
- **FR-04**: The product MUST return search results, graph facts, and versioned reads from the same selected View and identify that source state in each result. Every authorized code search, graph result, and versioned read MUST create one durable, append-only, scoped, non-content UCI exposure evidence record only after Source, Checkout, and View authorization and a closed result decision. The canonical exposure source is that shared MCP boundary decision. The canonical completion source is a verified supported-host callback. Exact idempotency retry returns the original record only when the canonical non-content binding matches; a mismatch returns non-disclosing `IDEMPOTENCY_MISMATCH`, creates nothing, and returns no stale receipt. A closed authorized `unavailable` decision with a healthy recorder MUST return its contextual envelope, closed source/index error, empty result items, and opaque exposure receipt. If initial exposure recording fails, the product MUST instead return `EXPOSURE_UNAVAILABLE` with `status: unavailable`, `exposure: null`, and no contextual envelope, result body, or items. A completion-write failure MUST return `COMPLETION_EVIDENCE_UNAVAILABLE` only to the callback without changing the original exposure or manufacturing completion. `completion_state` is closed: it is `unknown` without qualifying completion evidence, and a verified supported-host callback may establish only `succeeded`, `partial`, `failed`, or `abandoned`; it is independent of retrieval `result_state` and coverage. **Acceptance**: A combined search, graph, and read probe finds no result whose excerpt and relationship evidence refer to different Views; it finds one durable scoped exposure for every authorized closed response, including an authorized unavailable decision, proves that recorder failure and idempotency mismatch suppress both the contextual envelope and exposure, proves exact versus mismatched idempotency, and proves callback-backed completion remains separate from retrieval result and coverage.
...
- **Code Result**: A bounded, source-grounded excerpt or relationship answer that identifies its Source and View, match mode, coverage limitation, and a bounded opaque exposure reference. The exposure reference contains no source body or request content. Its closed `completion_state` is `unknown` without qualifying evidence, otherwise one verified supported-host outcome: `succeeded`, `partial`, `failed`, or `abandoned`; it does not restate retrieval `result_state` or coverage. A closed authorized unavailable outcome returns the selected contextual envelope, a closed error, empty result items, and its exposure receipt. An unavailable evidence append returns no contextual envelope, Code Result body, items, or exposure receipt.
- **UCI Evidence**: Durable, append-only, non-content exposure and completion evidence owned by the UCI recorder. It is backed up and integrity-verified through PostgreSQL, cannot be rebuilt from the code corpus, and never grants access, selects or publishes a View, or certifies product success.
...
- Native installation, two-client MCP proof, authorization-negative proof, measured acceptance against the approved product-task set, and durable UCI evidence with closed recorder failure and idempotency behavior.
