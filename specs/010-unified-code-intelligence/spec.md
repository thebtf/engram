...
- **FR-04**: The product MUST return search results, graph facts, and versioned reads from the same selected View and identify that source state in each result. Every authorized code search, graph result, and versioned read MUST create one idempotent, scoped, non-content UCI exposure only after Source, Checkout, and View authorization. Completion state MUST remain `unknown` without an actual supported-host callback, and only that callback may add completion evidence. **Acceptance**: A combined search, graph, and read probe finds no result whose excerpt and relationship evidence refer to different Views; it finds one idempotent scoped exposure for each authorized operation, none for a refusal, `unknown` with no callback, and a bound completion record only from a supported host.
