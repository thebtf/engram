...
| BrowserTabBinding | Server-issued opaque `tab_binding_id`; browser session reference; resume-nonce, current document-proof, and reload-token digests; document lease state/expiry; exact optional pinned `ContextRef`; binding expiry | Browser Binding Application owner; references BrowserSubject and UCI ContextRef | The normative browser lifecycle is the single transition table in `browser-context-and-grants.md`. Console Code makes fresh opener handling one-time by clearing copied storage, normalizing `window.opener` to null, and verifying the readback; only a normalized no-opener `reload` may resume, while normalized no-opener `navigate` with a copied pair collides and every failed/ambiguous classifier result is fresh/unselected. Every binding-bound Code request and lease operation presents the current binding ID plus document proof. Document lease expiry retains the binding/pin; only binding or session expiry destroys them. |
...
normalized no-opener `navigate` + copied pair -> TAB_BINDING_COLLISION -> fresh binding without ContextRef
opener normalization failure/persistence or ambiguous navigation bootstrap -> TAB_BOOTSTRAP_AMBIGUOUS -> fresh binding without ContextRef
