...
The Code UI classifies the bootstrap before calling either endpoint. On the recorded supported browser, `window.opener` present means a fresh opener: it clears copied Code Explorer storage and calls `handshake` without a pair. With no opener, `PerformanceNavigationTiming.type === "reload"` plus a valid pair selects `resume`; `type === "navigate"` plus a pair is the browser’s normal Duplicate/copy case and selects `handshake` with that pair. `back_forward`, absent/malformed/unknown navigation metadata, an unexpected opener/navigation tuple, or no valid pair is `ambiguous`: it clears the pair, calls fresh `handshake`, returns `TAB_BOOTSTRAP_AMBIGUOUS`, and never resumes a pin. Navigation metadata is a client routing signal, not document proof or authorization; the S2 fixture records its actual browser engine/version and observed opener/navigation tuple, and makes no automatic-resume/collision claim for a browser that does not exhibit these stated signals. None of the pair, proof, or token is an ACL, grant, URL value, log field, shared `localStorage` value, or MCP identity.
...
- Every binding-bound request and lease operation proves both its binding ID and its current document proof. The fixture records the browser engine/version, `window.opener`, and `PerformanceNavigationTiming.type` before bootstrap: an opener is fresh/no-collision, a real browser Duplicate/copy with no opener plus `navigate` receives `TAB_BINDING_COLLISION`, and only no-opener `reload` selects resume. It never selects an endpoint by fixture construction.
