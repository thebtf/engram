...
- Every binding-bound request and lease operation proves both its binding ID and its current document proof. Fresh openers clear copied storage and receive fresh unselected bindings; an explicit duplicate/copy preserves the pair, receives `TAB_BINDING_COLLISION`, receives a fresh unselected binding, and cannot alter the parent.
- Acknowledged-close hard reload retains each A/B pinned context. A delayed/missing lifecycle close returns `RELOAD_PENDING` without losing the pin, then resumes after close acknowledgement or document-lease expiry; a crash follows the expiry path. Replay of a consumed resume token and use of an old document proof fail closed without contextual output. Session or binding expiry destroys the pin and requires explicit re-selection.
