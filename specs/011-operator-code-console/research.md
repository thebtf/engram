...
**Decision**: Browser Binding Application owns the one deterministic transition table in `browser-context-and-grants.md`. It issues a distinct `tab_binding_id`, exchanges each fresh in-memory `document_nonce` for a current document proof, and requires the binding ID plus proof on every binding-bound Code request and lease operation. The server-owned, one-time reload-resume cookie binds the browser session, binding, and resume nonce, but is not tab-isolated evidence. The Code UI selects its endpoint from the actual bootstrap signals: `window.opener` means fresh/open-and-clear; no opener plus `PerformanceNavigationTiming.type === "reload"` resumes; no opener plus `type === "navigate"` with a pair is the supported browser Duplicate/copy path and handshakes to collision; all other/missing/ambiguous signals clear the pair and fresh-handshake without a pin. Navigation metadata is client routing, never authorization. A normal reload keeps its pinned context when a close is acknowledged; a delayed/missing close gets only `RELOAD_PENDING` until the live lease closes or expires, after which the same one-time resume restores the binding and pin. Document-lease expiry retains that binding/pin; session or binding expiry destroys them. Token/proof replay fails closed.

**Rationale**: `sessionStorage` is copied to an opener-created page (MDN: <https://developer.mozilla.org/en-US/docs/Web/API/Window/sessionStorage>), so a client-chosen stored key cannot prove tab uniqueness. Lifecycle notification may be delayed or absent, so reload cannot depend on a close arrival. A cookie is host/path scoped rather than tab scoped and therefore cannot choose between duplicate and reload. The actual browser navigation entry and opener state are an observable bootstrap classification for the declared fixture, not a security proof; absent or unexpected values deliberately fail closed. `internal/uci/context_resolver.go:14-16,39-52` shows that mutable selection is client-scoped convenience and explicit authorization must not change a client default. The document proof identifies one document within an authenticated subject only; exact BrowserReadGrant and UCI ContextRef checks remain separately authoritative.

**Rejected alternatives**: global current worktree; `sessionStorage` uniqueness by assertion; a shared localStorage key; treating the reload cookie as per-tab evidence; silently rotating an existing tab; URL bearer values; a resume that trusts lifecycle close arrival; treating navigation metadata or document proof as code authorization; or failing a hard reload solely because a fresh document nonce is expected.

**Implementation boundary**: Browser Binding Application is the exclusive writer for binding persistence, document lease/resume/collision state machine, and focused Go tests. The Console Code owner owns the bootstrap classifier, captures the engine/version and opener/navigation trace in scenario evidence, and sends its documented headers/results; it clears copied storage only for a fresh opener and adds S2 scenario assertions through the S1a-owned harness without changing its config/script/bootstrap. The binding owner receives canonical BrowserSubject/session carriage from Auth/grants, hands transition/guard operations to HTTP, and a persistence declaration to service composition.
