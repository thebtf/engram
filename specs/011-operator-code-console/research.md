...
**Decision**: Browser Binding Application owns the one deterministic transition table in `browser-context-and-grants.md`. It issues a distinct `tab_binding_id`, exchanges each fresh in-memory `document_nonce` for a current document proof, and requires the binding ID plus proof on every binding-bound Code request and lease operation. A server-issued, one-time reload-resume cookie binds the browser session, binding, and resume nonce. A normal reload keeps its pinned context when a close is acknowledged; a delayed/missing close gets only `RELOAD_PENDING` until the live lease closes or expires, after which the same one-time resume restores the binding and pin. Document-lease expiry retains that binding/pin; session or binding expiry destroys them. A fresh opener clears copied storage and receives a fresh binding; an explicit duplicate/copy follows handshake with the copied pair, receives `TAB_BINDING_COLLISION`, and rotates only the new document to a fresh unselected binding. Token/proof replay fails closed.

**Rationale**: `sessionStorage` is copied to an opener-created page (MDN: <https://developer.mozilla.org/en-US/docs/Web/API/Window/sessionStorage>), so a client-chosen stored key cannot prove tab uniqueness. Lifecycle notification may be delayed or absent, so reload cannot depend on a close arrival. `internal/uci/context_resolver.go:14-16,39-52` shows that mutable selection is client-scoped convenience and explicit authorization must not change a client default. The document proof identifies one document within an authenticated subject only; exact BrowserReadGrant and UCI ContextRef checks remain separately authoritative.

**Rejected alternatives**: global current worktree; `sessionStorage` uniqueness by assertion; a shared localStorage key; silently rotating an existing tab; URL bearer values; a resume that trusts lifecycle close arrival; treating a document proof as code authorization; or failing a hard reload solely because a fresh document nonce is expected.

**Implementation boundary**: Browser Binding Application is the exclusive writer for binding persistence, document lease/resume/collision state machine, and focused Go tests. It receives canonical BrowserSubject/session carriage from Auth/grants; hands transition/guard operations to HTTP, a persistence declaration to service composition, and documented headers/results to the Code UI. The Console Code owner consumes those results, clears copied storage only for a fresh opener, and adds S2 scenario assertions through the S1a-owned harness without changing its config/script/bootstrap.
