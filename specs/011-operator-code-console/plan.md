...
- **Browser identity**: a persistent real BrowserSubject, exact Source+Checkout grant, server-issued `tab_binding_id`, and current per-document proof. Browser `sessionStorage` retains only a resume pair; the server-owned one-time reload token is not an authorization artifact. Binding/proof identify a document within an authenticated subject; the grant and UCI ContextRef remain separate read authority.
...
| S2 typed Code UI | Console Code owner | `useOperatorCode.ts`, Code page/components, tab resume storage/current document proof, code-specific live browser scenario tests | Consumes the Browser Binding Application transition/guard contract and HTTP DTOs only. Cannot edit binding persistence, MCP binding, grant store, UCI graph, or shared harness config/script. |
...
| 4 — copied tab storage isolation | `contracts/browser-context-and-grants.md:36-71`; `data-model.md:9,18,35-47,80,84`; `research.md:25-33`; `quickstart.md:42,46-50`; `plan.md:20,43,52,102-108,121` | One deterministic binding table requires binding ID plus current document proof on binding-bound requests/leases; fresh openers clear copied state, explicit copies rotate only the new tab, server-owned one-time resume retains normal reload pin after close/lease expiry, and replay/expiry fail closed. Document lease expiry retains binding/pin; only session/binding expiry destroys them. |
