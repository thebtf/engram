...
| T002 | Code | Foundation maker: `internal/recoveryinventory/flag_scan.go` and tests | T001 common record schema | Every current reader/default is recorded with parsing/default semantics; no runtime state is asserted. |
| T003 | Code | Foundation maker: `internal/recoveryinventory/surface_scan.go` and tests | T001 common record schema | Route, MCP, daemon, hook, tool, current documentation claim, and claim-only inventory of both `ui/` and `apps/operator-console/` roots are classified with source evidence; neither surface is changed. |
| T004 | Code | Foundation maker: `internal/recoveryinventory/project_data_scan.go` and tests | T001 common record schema | Relational, multi-role, serialized/cache/job/import/export families are enumerated or source-uncertain; no data rows are read. |
| T005 | Code | Foundation maker: `internal/operability/baseline_metrics.go` and tests | T001–T004 inventory facts | Each metric uses named numerator/denominator/scope/window/freshness and emits `not_computable` for zero denominators. |
...
| T008 | Test | Fence maker: `internal/projectidentity/legacy_fence_test.go` plus narrowly needed existing adapter tests | None; RED before T009 | Unknown project-only HTTP/gRPC/store requests create no project/alias; known unique selector resolution remains covered. |
| T009 | Code | Fence maker: `internal/db/gorm/project_store.go` | T008 RED evidence | Only the selector-only zero-candidate creation arm refuses before mutation; identity-present V2 behavior remains intact. |
...
| T013 | Code | Receipt maker: `internal/recoveryreceipt/ar1_baseline.go` and tests | Foundation T001–T005 plus T012 | The sole AR-1 baseline receipt producer validates/consumes scenario evidence plus source, metric, and health facts, then binds truthful denominator states. |
...
| T015 | Test | Gate runner plus installed-dogfood observer; no source edits | Checker/review/judge clean exact head | Clean exact-head build, applicable tests, isolated fixture, staged built payload, AR-1 scenario, observation receipt, and rollback boundary succeed. |
...
- **Receipt lane (T013):** cannot precede its upstream facts and is meaningful
  only as the one integration receipt consumed by T015.
...
| FR-019, FR-020, SC-005 boundary | T005, T011–T013, T015 | `internal/operability/baseline_metrics.go`; `scripts/recovery/**`; `internal/recoveryreceipt/ar1_baseline.go` |
