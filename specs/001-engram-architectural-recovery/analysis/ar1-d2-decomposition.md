...
| T009 | Code | Identity-security maker: `internal/db/gorm/project_store.go`, `internal/proxy/identity.go`, and focused tests | T008 RED evidence | Selector-only zero-candidate creation refuses before mutation; credential-bearing Git remote userinfo is rejected or reduced before any durable identity write; identity-present V2 behavior remains otherwise intact. |
