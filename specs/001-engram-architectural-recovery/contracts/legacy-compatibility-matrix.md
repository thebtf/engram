...
For an approved single-row historical adoption, the compatibility record MUST name the last V2 read boundary and each supported client version, selector, read and mutation route, and serialized field. `RegisterAnchorIntentV3` MUST fence and check inventoried historical ownership before fresh registration; an unbound V3 anchor with possible or unproven historical ownership receives a typed adoption hold, never a fresh row. Only a proven genuinely new scope may register; raw name, remote, path, hash, or selector evidence never auto-matches an owner. Read-only preflight accepts only a validated descriptor and a server-issued, owner-scoped opaque historical-row selection reference, not a raw selector, path, name, remote, hash, or `project_key`. It does not mint a UUID, modify the old row, or enumerate owners on success or refusal. Until every affected role is reconciled and both paths switch under the same write fence, V3 refuses and V2 reads remain at the declared compatible boundary; divergent TEXT-only writes are refused. After cutover, a supported V2 selector resolves the adopted row's server-issued UUID before reading or writing canonical stores. A required historical TEXT projection is read-only and versioned; unsupported versions or unclassified fields refuse. A completed adoption receipt records the old-row evidence reference, plan, approvals, verified sealed export, successful V2 readback, unique per-family and per-role count and semantic evidence, quarantine, and rehearsed rollback at the last compatible boundary without fabricating a source UUID or merge manifest. The validator holds cutover unless every inventoried role is covered exactly once, before equals migrated plus quarantined plus native-global, after equals migrated, and referenced integrity evidence verifies. Adoption refusals use the distinct `PROJECT_ADOPTION_*` outcomes in `project-identity-v3.md`, with no canonical key or candidate data returned.
