...
DO NOT EXTRACT: File reads without decisions, routine commits, tool invocations without meaningful output, status checks, version bumps, generic descriptions of what code does, task checkbox toggles (changing [ ] to [x] or [x] to [ ] in tasks.md, TODO.md, or checklists — these are routine progress tracking, not decisions).
