...
func TestComparisonObservationV3Classification(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			observation, err := NewComparisonObservationV3(ComparisonObservationInputV3{IdempotencyKey: key, Correlation: correlation, V3Outcome: test.v3Outcome, LegacyOutcome: test.legacy, ClientInstanceID: "comparison-client-" + uuid.NewString(), Transport: ComparisonTransportGRPCV3, Scope: ComparisonRepositoryScopeV3, Freshness: ComparisonFreshV3, EvidenceFingerprint: evidence})
			require.NoError(t, err)
...
func TestComparisonObservationV3RejectsUnsafeInputs(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			_, err := NewComparisonObservationV3(ComparisonObservationInputV3{IdempotencyKey: test.idempotency, Correlation: correlation, V3Outcome: ProjectDescriptorInvalidOutcomeV3, LegacyOutcome: LegacyComparisonRefusalV2, ClientInstanceID: test.client, Transport: ComparisonTransportHookV3, Scope: ComparisonDirectoryScopeV3, Freshness: ComparisonFreshV3, EvidenceFingerprint: test.evidence})
			require.Error(t, err)
		})
	}
}
