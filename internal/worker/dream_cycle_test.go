...
func TestDreamCycle_RealDBRestartRetryPersistsExactlyOneCandidate(t *testing.T) {
	...
	require.NoError(t, ts1.Create(context.Background(), &gorm.SessionTranscript{
		SessionID: marker,
		Project:   marker,
	...
	first := &Service{
		dreamTranscriptStoreOverride: &markFailOnceTranscriptStore{dreamTranscriptStore: ts1},
		dreamCandidateStoreOverride:  cs1,
		dreamExtractorFunc:           extractFn,
	}
	first.runDreamCrystallization(context.Background())
	...
	require.NoError(t, err)
	restarted := &Service{
		dreamTranscriptStoreOverride: gorm.NewTranscriptStore(store2.DB),
		dreamCandidateStoreOverride:  gorm.NewCandidateStore(store2.DB, gorm.NewAuditStore(store2.DB)),
		dreamExtractorFunc:           extractFn,
	}
	restarted.runDreamCrystallization(context.Background())
	...
	require.NoError(t, err)
	afterSuccess := &Service{
		dreamTranscriptStoreOverride: gorm.NewTranscriptStore(store3.DB),
		dreamCandidateStoreOverride:  gorm.NewCandidateStore(store3.DB, gorm.NewAuditStore(store3.DB)),
		dreamExtractorFunc:           extractFn,
	}
	afterSuccess.runDreamCrystallization(context.Background())
