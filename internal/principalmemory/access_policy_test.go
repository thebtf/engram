...
func TestPrincipalAccessPolicy_PrivacyAndAuditDecisions(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:        "empty target principal kind is rejected",
			caller:      PrincipalRef{Principal: "agent/bob", PrincipalKind: "agent"},
			target:      PrincipalRef{Principal: "agent/alice", PrincipalKind: ""},
			visibility:  "shared",
			wantAllowed: false,
			wantAudit:   false,
			wantReason:  "invalid_principal_kind",
		...
		{
			name:        "empty visibility defaults to shared",
			caller:      PrincipalRef{Principal: "", PrincipalKind: ""},
			target:      PrincipalRef{Principal: "agent/alice", PrincipalKind: "agent"},
			visibility:  "",
			wantAllowed: true,
			wantAudit:   false,
		...
		{
			name:        "invalid caller principal kind is rejected for private",
			caller:      PrincipalRef{Principal: "agent/alice", PrincipalKind: "robot"},
			target:      PrincipalRef{Principal: "agent/alice", PrincipalKind: "agent"},
			visibility:  "private",
			wantAllowed: false,
			wantAudit:   false,
		...
		{
			name:        "empty caller principal kind cannot self-match private",
			caller:      PrincipalRef{Principal: "agent/alice", PrincipalKind: ""},
			target:      PrincipalRef{Principal: "agent/alice", PrincipalKind: "agent"},
			visibility:  "private",
			wantAllowed: false,
			wantAudit:   false,
		...
		{
			name:        "principal kind whitespace and case normalize before match",
			caller:      PrincipalRef{Principal: "agent/alice", PrincipalKind: "  AGENT  "},
			target:      PrincipalRef{Principal: "agent/alice", PrincipalKind: "agent"},
			visibility:  "private",
			wantAllowed: true,
