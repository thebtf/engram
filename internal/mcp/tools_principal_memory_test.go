...
func TestQueryPrincipalMemory_ToolSchemaAdvertised(t *testing.T) {
	...
	for _, field := range []string{"principal", "principal_kind", "project", "domain", "q", "visibility", "include_private", "limit", "offset", "session_id"} {
		assert.Contains(t, props, field)
	}
	required := tool.InputSchema["required"].([]string)
	assert.ElementsMatch(t, []string{"principal", "principal_kind"}, required)
}
