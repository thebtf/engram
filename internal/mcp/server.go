...
type Server struct {
	...
	nodesStore                    nodesStoreAPI // retained historical graph reader filter
...
// SetNodesStore wires the NodesStore used by retained graph readers.
...
func (s *Server) handleToolsList(req *Request) *Response {
	...
	// Historical graph reads remain available without a legacy feature flag.
	if s.graphStore != nil {
		tools = append(tools, Tool{
			Name:        "graph",
			Description: "[Graph] Read historical graph edges, traversals, paths, and synonyms.",
			tier:        tierUseful,
			InputSchema: map[string]any{
				"type":     "object",
				"required": []string{"action"},
				"properties": map[string]any{
					"action":     map[string]any{"type": "string", "description": "Action: get_edges, traverse, find_path, synonyms", "enum": []string{"get_edges", "traverse", "find_path", "synonyms"}},
					"source_id":  map[string]any{"type": "integer", "description": "Source memory ID for find_path"},
					"target_id":  map[string]any{"type": "integer", "description": "Target memory ID for find_path"},
					"memory_id":  map[string]any{"type": "integer", "description": "Memory ID for get_edges, traverse, or synonyms"},
					"node_id":    map[string]any{"type": "integer", "description": "Node ID for get_edges"},
					"node_type":  map[string]any{"type": "string", "description": "Node type filter for get_edges"},
					"edge_type":  map[string]any{"type": "string", "description": "Relationship type filter for get_edges"},
					"edge_types": map[string]any{"type": "array", "items": map[string]any{"type": "string"}, "description": "Relationship type filters for traverse"},
					...
					"max_depth":  map[string]any{"type": "integer", "description": "Maximum path depth for find_path (1-3)"},
				},
			},
		})
	}
