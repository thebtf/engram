...
func TestReadOnlyToolAllowed_DispatchAndActionInventory(t *testing.T) {
	allowed := []struct{ name, args string }{
		{"recall", `{}`},
		{"recall", `{"action":"search"}`},
		{"docs", `{"action":"read"}`},
		{"docs", `{"action":"list"}`},
		{"docs", `{"action":"history"}`},
		{"docs", `{"action":"collections"}`},
		...
		{"lifecycle", `{"action":"sleep_status"}`},
		{"lifecycle", `{"action":"decay_preview"}`},
		{"graph", `{"action":"get_edges"}`},
		{"graph", `{"action":"traverse"}`},
		{"graph", `{"action":"find_path"}`},
		{"graph", `{"action":"synonyms"}`},
		{"check_system_health", `{}`},
		{"list_rules", `{}`},
		{"recall_memory", `{}`},
		{"know_about", `{}`},
		{"query_principal_memory", `{}`},
		{"experience_history.read", `{}`},
		{"experience_history.detail", `{}`},
		{"temporal_truth", `{}`},
		{"get_memory_brief", `{}`},
		{"list_candidates", `{}`},
		{"get_candidate", `{}`},
	...
	denied := []struct{ name, args string }{
		...
		{"docs", `{"action":"future"}`},
		{"admin", `{"action":"stats"}`},
		{"admin", `{"action":"purge_project"}`},
		{"issues", `{"action":"create"}`},
		{"issues", `{"action":"update"}`},
		{"issues", `{"action":"comment"}`},
		{"issues", `{"action":"reopen"}`},
		{"issues", `{"action":"close"}`},
		{"issues", `{"action":"future"}`},
		{"lifecycle", `{"action":"promote"}`},
		{"lifecycle", `{"action":"demote"}`},
		...
		{"get_state", `{"action":"session"}`},
		{"get_state", `{"action":"project"}`},
		{"get_state", `{"action":"resume"}`},
		{"set_state", `{"action":"session"}`},
		{"set_state", `{"action":"project"}`},
		{"temporal_truth_refresh", `{}`},
		{"remember_directive", `{}`},
		{"rate_memory_significance", `{}`},
		{"get_ambient_hints", `{}`},
		{"promote_candidate", `{}`},
		{"reject_candidate", `{}`},
		{"supersede_candidate", `{}`},
		...
		{"rule_governance_transition", `{}`},
		{"rule_governance_pin_snapshot", `{}`},
		{"rule_governance_rollback", `{}`},
		{"bulk_promote", `{}`},
		{"bulk_delete", `{}`},
		{"bulk_supersede", `{}`},
		{"future_tool", `{}`},
