...
func TestStrictMutationArgumentsRejectLossyOrPartialValues(t *testing.T) {
	...
	for _, tc := range []struct{ raw, key string }{{`{"dry_run":null}`, "dry_run"}, {`{"dry_run":"false"}`, "dry_run"}, {`{"tags":null}`, "tags"}, {`{"tags":["safe",1]}`, "tags"}, {`{"supersedes":null}`, "supersedes"}, {`{"supersedes":[1,"2"]}`, "supersedes"}, {`{"supersedes":[1,2.5]}`, "supersedes"}} {
...
func TestParseArgsRejectsTrailingJSONValues(t *testing.T) {
	for _, raw := range []string{`{"id":1}{"id":2}`, `{"id":1} null`, `{"id":1} 2`} {
...
func FuzzRequireInt64ArgRejectsLossyRepresentations(f *testing.F) {
	for _, seed := range []string{"0", "-0", "1", "-1", "9007199254740993", "9223372036854775807", "9223372036854775808", "1.5", "1e3", "1E3", `"42"`, "null", "true"} {
		f.Add(seed)
	}
...
func TestMutationSelectorsPreserveExactNumbersAndRejectLoss(t *testing.T) {
	m, err := parseArgs(json.RawMessage(`{"id":9007199254740993,"ids":[1,9007199254740993]}`))
	if err != nil {
		t.Fatal(err)
	}
	if got, err := requireInt64Arg(m, "id"); err != nil || got != 9007199254740993 {
		t.Fatalf("exact id = %d, err = %v", got, err)
	}
	if got, _, err := optionalInt64SliceArg(m, "ids"); err != nil || len(got) != 2 || got[1] != 9007199254740993 {
		t.Fatalf("exact ids = %v, err = %v", got, err)
	}
	for _, raw := range []string{`{"id":"42"}`, `{"id":1.5}`, `{"id":1e3}`, `{"id":9223372036854775808}`} {
		m, err := parseArgs(json.RawMessage(raw))
		if err != nil {
			t.Fatal(err)
		}
		if _, err := requireInt64Arg(m, "id"); err == nil {
			t.Fatalf("lossy selector accepted: %s", raw)
		}
	}
}
