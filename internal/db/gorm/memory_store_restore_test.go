...
func TestMemoryStore_RestoreRawTx_RestoresCompleteSnapshot(t *testing.T) {
	...
	require.Equal(t, before, restored, "rollback must restore the complete persisted snapshot")
}

func TestMemoryStore_RestoreRawTx_EmptySourceSessionsRemainNonNull(t *testing.T) {
	...
	row := &Memory{
		Project:        "restore-raw-empty-sessions",
		Content:        "empty source sessions snapshot",
		SourceAgent:    "test",
		SourceSessions: pq.StringArray{},
	}
	require.NoError(t, db.Create(row).Error)
	t.Cleanup(func() {
		_ = db.Unscoped().Delete(&Memory{}, "id = ?", row.ID).Error
	})

	snapshot, err := store.GetForSnapshot(ctx, row.ID)
	require.NoError(t, err)
	encodedSnapshot, err := json.Marshal(snapshot)
	require.NoError(t, err)
	var before models.Memory
	require.NoError(t, json.Unmarshal(encodedSnapshot, &before))
	require.Nil(t, before.SourceSessions, "empty source_sessions is omitted from JSON snapshots")

	require.NoError(t, db.Model(&Memory{}).Where("id = ?", row.ID).Update("source_sessions", pq.StringArray{"mutated-session"}).Error)
	require.NoError(t, db.WithContext(ctx).Transaction(func(tx *gormlib.DB) error {
		return store.RestoreRawTx(ctx, tx, &before)
	}))

	var isNull bool
	require.NoError(t, db.Raw("SELECT source_sessions IS NULL FROM memories WHERE id = ?", row.ID).Scan(&isNull).Error)
	assert.False(t, isNull, "restoring an omitted empty source_sessions snapshot must write '{}' rather than NULL")
	var restored Memory
	require.NoError(t, db.Where("id = ?", row.ID).First(&restored).Error)
	require.NotNil(t, restored.SourceSessions)
	assert.Empty(t, restored.SourceSessions)
}
