	...
	return &gormigrate.Migration{
		...
		Migrate: func(tx *gormlib.DB) error {
			stmts := []string{
				...
				`ALTER TABLE invitations ADD COLUMN IF NOT EXISTS revoked_by BIGINT REFERENCES users(id)`,
				...
				`ALTER TABLE sessions ADD COLUMN IF NOT EXISTS revoked_by BIGINT REFERENCES users(id)`,
