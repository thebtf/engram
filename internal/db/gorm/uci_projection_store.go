...
const (
	uciProjectionMaxSourceTextBytes   = 64 << 10
	uciProjectionArtifactIDWhere      = "artifact_id = ?"
	uciProjectionSourceArtifactWhere  = "source_id = ? AND artifact_id = ?"
	uciProjectionProfileIDWhere       = "profile_id = ?"
	uciProjectionCheckoutIDWhere      = "checkout_id = ?"
	uciProjectionViewIDWhere          = "view_id = ?"
	uciProjectionJobIDWhere           = "job_id = ?"
	uciProjectionBuildIDWhere         = "build_id = ?"
	uciProjectionDigestPrefix         = "sha256:"
	uciHybridConditionSeparator       = "\n\t\t\t\tAND "
	uciHybridSourceIDColumn           = ".source_id"
	uciHybridArtifactIDColumn         = ".artifact_id"
	uciHybridChunkContentDigestColumn = ".chunk_content_digest"
	uciHybridEntityKeyColumn          = ".entity_key"
	uciHybridRelativePathColumn       = ".relative_path"
	uciHybridByteStartColumn          = ".byte_start"
	uciHybridByteEndColumn            = ".byte_end"
)
...
func (s *UCIProjectionStore) UpsertParseArtifact(ctx context.Context, in UpsertUCIParseArtifactInput) (*UCIParseArtifact, error) {
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) && in.ArtifactID != "" {
			...
			occupiedErr := s.db.WithContext(ctx).Where(uciProjectionArtifactIDWhere, in.ArtifactID).First(&occupied).Error
			if occupiedErr == nil {
				return nil, errUCIProjectionImmutable
			}
...
const (
	...
	uciIndexAdmissionAnalyzeThreshold = int64(10_000)
)
...
func (s *UCIProjectionStore) AdmitIndexFrames(ctx context.Context, sourceID, profileID string, frames []ucidomain.IndexAdmissionFrame) ([]ucidomain.IndexPart, error) {
	...
	canonicalFrames, err := validateUCIIndexAdmissionFrames(ctx, sourceID, profileID, frames)
	if err != nil {
		return nil, err
	}
	var admitted []ucidomain.IndexPart
	var insertedFactRows int64
	err = s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		var transactionErr error
		admitted, insertedFactRows, transactionErr = s.admitUCIIndexAdmissionFramesTx(ctx, tx, sourceID, profileID, canonicalFrames)
...
func validateUCIIndexAdmissionFrames(ctx context.Context, sourceID, profileID string, frames []ucidomain.IndexAdmissionFrame) ([]ucidomain.IndexAdmissionFrame, error) {
	...
	for index, frame := range frames {
		...
		if err := validateUCIIndexAdmissionFrame(sourceID, profileID, canonical); err != nil {
			return nil, err
		}
	...
	return canonicalFrames, nil
}

func validateUCIIndexAdmissionFrame(sourceID, profileID string, frame ucidomain.IndexAdmissionFrame) error {
	if frame.Profile.ID != profileID {
		return fmt.Errorf("uci index admission: frame profile does not match authorized profile")
	}
	for _, artifact := range frame.Artifacts {
		if err := validateUCIIndexAdmissionArtifact(sourceID, artifact); err != nil {
			return err
		}
	}
	return nil
...
func validateUCIIndexAdmissionArtifact(sourceID string, artifact ucidomain.IndexAdmissionArtifact) error {
	expectedID, err := ucidomain.DeriveIndexAdmissionArtifactID(sourceID, artifact.ContentDigest, artifact.Profile)
	if err != nil {
		return fmt.Errorf("uci index admission: derive artifact ID: %w", err)
	}
	if artifact.ArtifactID != expectedID {
		return fmt.Errorf("uci index admission: artifact ID does not match authorized source")
	}
...
func (s *UCIProjectionStore) admitUCIIndexAdmissionFramesTx(ctx context.Context, tx *gorm.DB, sourceID, profileID string, frames []ucidomain.IndexAdmissionFrame) ([]ucidomain.IndexPart, int64, error) {
	store := &UCIProjectionStore{db: tx}
	bindings, err := store.uciIndexAdmissionReferenceBindings(ctx, profileID, frames)
	if err != nil {
		return nil, 0, err
	}
	proofs, insertedFactRows, err := store.admitUCIIndexAdmissionArtifacts(ctx, sourceID, frames, bindings)
	if err != nil {
		return nil, 0, err
	}
	admitted, err := uciIndexAdmissionPartsWithProofs(frames, bindings, proofs)
	if err != nil {
		return nil, 0, err
	}
	return admitted, insertedFactRows, nil
}

func (s *UCIProjectionStore) uciIndexAdmissionReferenceBindings(ctx context.Context, profileID string, frames []ucidomain.IndexAdmissionFrame) (ucidomain.IndexAdmissionReferenceBindings, error) {
	bindings := make(ucidomain.IndexAdmissionReferenceBindings)
	for _, frame := range frames {
		if err := ctx.Err(); err != nil {
			return nil, err
		}
		if err := s.validateUCIIndexAdmissionProfile(ctx, profileID, frame); err != nil {
			return nil, err
		}
		frameBindings, err := frame.ReferenceBindings()
		if err != nil {
			return nil, fmt.Errorf("uci index admission: derive reference bindings: %w", err)
		}
...
func (s *UCIProjectionStore) admitUCIIndexAdmissionArtifacts(ctx context.Context, sourceID string, frames []ucidomain.IndexAdmissionFrame, bindings ucidomain.IndexAdmissionReferenceBindings) (map[string]ucidomain.IndexArtifactProof, int64, error) {
	...
	for _, frame := range frames {
		for _, artifact := range frame.Artifacts {
			...
			proof, inserted, err := s.admitUCIIndexAdmissionArtifact(ctx, sourceID, artifact, bindings)
			if err != nil {
				return nil, 0, err
			}
			insertedFactRows += inserted
			proofs[artifact.ArtifactID] = proof
		}
	}
	return proofs, insertedFactRows, nil
}

func uciIndexAdmissionPartsWithProofs(frames []ucidomain.IndexAdmissionFrame, bindings ucidomain.IndexAdmissionReferenceBindings, proofs map[string]ucidomain.IndexArtifactProof) ([]ucidomain.IndexPart, error) {
	admitted := make([]ucidomain.IndexPart, len(frames))
	for frameIndex, frame := range frames {
		part, err := frame.IndexPart(bindings)
		if err != nil {
			return nil, fmt.Errorf("uci index admission: canonical publication part: %w", err)
		}
		for artifactIndex := range part.Artifacts {
			proof, found := proofs[part.Artifacts[artifactIndex].ArtifactID]
			if !found {
				return nil, fmt.Errorf("uci index admission: missing stored artifact proof")
			}
			part.Artifacts[artifactIndex] = proof
		}
		if _, err := ucidomain.DigestIndexPart(part); err != nil {
			return nil, fmt.Errorf("uci index admission: validate stored publication part: %w", err)
		}
...
func (s *UCIProjectionStore) refreshUCIProjectionStatistics(ctx context.Context) error {
	if err := s.db.WithContext(ctx).Exec("ANALYZE ci_blobs, ci_parse_artifacts, ci_definitions, ci_reference_sites, ci_chunks").Error; err != nil {
		return fmt.Errorf("uci index admission: refresh projection statistics: %w", err)
	}
	return nil
}
...
func (s *UCIProjectionStore) validateUCIIndexAdmissionProfile(ctx context.Context, profileID string, frame ucidomain.IndexAdmissionFrame) error {
	...
	if err := s.db.WithContext(ctx).Where(uciProjectionProfileIDWhere, profileID).First(&profile).Error; err != nil {
...
func (s *UCIProjectionStore) admitUCIIndexAdmissionArtifact(ctx context.Context, sourceID string, artifact ucidomain.IndexAdmissionArtifact, bindings ucidomain.IndexAdmissionReferenceBindings) (ucidomain.IndexArtifactProof, int64, error) {
	status, err := uciIndexAdmissionArtifactStatus(artifact.Status)
	if err != nil {
		return ucidomain.IndexArtifactProof{}, 0, err
	}
	...
	if err != nil {
		return ucidomain.IndexArtifactProof{}, 0, fmt.Errorf("uci index admission: upsert blob: %w", err)
	}
	...
	if err != nil {
		return ucidomain.IndexArtifactProof{}, 0, fmt.Errorf("uci index admission: upsert artifact: %w", err)
	}
	if stored.ArtifactID != artifact.ArtifactID || stored.SourceID != sourceID {
		return ucidomain.IndexArtifactProof{}, 0, errUCIProjectionImmutable
	}

	insertedFactRows := int64(0)
	if stored.SealedAt == nil {
		insertedFactRows, err = s.storeUCIIndexAdmissionFacts(ctx, sourceID, artifact, bindings)
		if err != nil {
			return ucidomain.IndexArtifactProof{}, 0, err
		}
	}
	...
	if err != nil {
		return ucidomain.IndexArtifactProof{}, 0, fmt.Errorf("uci index admission: describe stored artifact: %w", err)
	}
	return proof, insertedFactRows, nil
}

func (s *UCIProjectionStore) storeUCIIndexAdmissionFacts(ctx context.Context, sourceID string, artifact ucidomain.IndexAdmissionArtifact, bindings ucidomain.IndexAdmissionReferenceBindings) (int64, error) {
	stored, err := s.loadMutableUCIArtifact(ctx, artifact.ArtifactID)
	if err != nil {
		return 0, err
	}
	if stored.SourceID != sourceID {
		return 0, fmt.Errorf("uci index admission: artifact source mismatch")
	}
	...
	for _, reference := range artifact.References {
		...
		if !found {
			return 0, fmt.Errorf("uci index admission: missing reference binding")
		}
	...
	for _, batch := range []struct {
		...
		{name: "definitions", rows: &definitions, count: len(definitions)},
		{name: "reference sites", rows: &references, count: len(references)},
		{name: "chunks", rows: &chunks, count: len(chunks)},
	} {
		if batch.count == 0 {
			continue
		}
		result := s.db.WithContext(ctx).Clauses(clause.OnConflict{DoNothing: true}).CreateInBatches(batch.rows, uciIndexAdmissionFactBatchSize)
		if result.Error != nil {
			return 0, fmt.Errorf("uci index admission: store %s: %w", batch.name, result.Error)
		}
...
func uciIndexAdmissionJSONYAMLDefinitionName(language ucidomain.IndexAdmissionLanguage, definition ucidomain.IndexAdmissionDefinition) (string, error) {
	...
	document, suffix, hasSuffix := strings.Cut(strings.TrimPrefix(definition.LocalSymbolKey, prefix), "#")
	...
	return uciIndexAdmissionJSONYAMLDefinitionSuffixName(language, definition, suffix)
}

func uciIndexAdmissionJSONYAMLDefinitionSuffixName(language ucidomain.IndexAdmissionLanguage, definition ucidomain.IndexAdmissionDefinition, suffix string) (string, error) {
	if suffix == "" {
		return "", fmt.Errorf("uci index admission: unparseable %s definition key %q", language, definition.LocalSymbolKey)
	}
...
func (s *UCIProjectionStore) verifyUCIIndexAdmissionFacts(ctx context.Context, artifact ucidomain.IndexAdmissionArtifact, bindings ucidomain.IndexAdmissionReferenceBindings) error {
	if err := s.verifyUCIIndexAdmissionDefinitions(ctx, artifact); err != nil {
		return err
	}
	if err := s.verifyUCIIndexAdmissionReferences(ctx, artifact, bindings); err != nil {
		return err
	}
	return s.verifyUCIIndexAdmissionChunks(ctx, artifact)
}

func (s *UCIProjectionStore) verifyUCIIndexAdmissionDefinitions(ctx context.Context, artifact ucidomain.IndexAdmissionArtifact) error {
	var definitions []UCIDefinition
	if err := s.db.WithContext(ctx).Where(uciProjectionArtifactIDWhere, artifact.ArtifactID).Find(&definitions).Error; err != nil {
...
func (s *UCIProjectionStore) verifyUCIIndexAdmissionReferences(ctx context.Context, artifact ucidomain.IndexAdmissionArtifact, bindings ucidomain.IndexAdmissionReferenceBindings) error {
	var references []UCIReferenceSite
	if err := s.db.WithContext(ctx).Where(uciProjectionArtifactIDWhere, artifact.ArtifactID).Find(&references).Error; err != nil {
...
func (s *UCIProjectionStore) verifyUCIIndexAdmissionChunks(ctx context.Context, artifact ucidomain.IndexAdmissionArtifact) error {
	var chunks []UCIChunk
	if err := s.db.WithContext(ctx).Where(uciProjectionArtifactIDWhere, artifact.ArtifactID).Order("ordinal ASC").Find(&chunks).Error; err != nil {
...
func (s *UCIProjectionStore) SelectHybridCandidates(ctx context.Context, authorized ucidomain.AuthorizedContext, profile ucidomain.VectorProfile, vector []float32, spec ucidomain.QuerySpec) (ucidomain.SemanticStoreResult, error) {
	ref, err := s.validateUCIHybridSelection(authorized, profile, vector, spec)
	if err != nil {
	...
	err = s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		result, err = selectUCIHybridCandidatesTx(tx, ref, profile, vector, spec)
		return err
...
func (s *UCIProjectionStore) validateUCIHybridSelection(authorized ucidomain.AuthorizedContext, profile ucidomain.VectorProfile, vector []float32, spec ucidomain.QuerySpec) (ucidomain.ContextRef, error) {
	if err := s.requireDB("select hybrid candidates"); err != nil {
		return ucidomain.ContextRef{}, err
	}
	ref := authorized.Ref()
...
func selectUCIHybridCandidatesTx(tx *gorm.DB, ref ucidomain.ContextRef, profile ucidomain.VectorProfile, vector []float32, spec ucidomain.QuerySpec) (ucidomain.SemanticStoreResult, error) {
	metadata, found, err := loadUCIQueryViewMetadata(tx, ref)
	if err != nil {
		return ucidomain.SemanticStoreResult{}, err
	}
	result, ready := uciHybridMetadataResult(metadata, found, spec)
	if !ready {
		return result, nil
	}
	...
	if err := tx.Raw(coverageQuery, coverageArguments...).Scan(&coverageRow).Error; err != nil {
		return ucidomain.SemanticStoreResult{}, fmt.Errorf("uci projection select hybrid coverage: %w", err)
	}
	if coverageRow.TotalCandidates < 0 || coverageRow.CompatibleCandidates < 0 || coverageRow.CompatibleCandidates > coverageRow.TotalCandidates {
		return ucidomain.SemanticStoreResult{}, fmt.Errorf("uci projection select hybrid coverage: invalid counts")
	}
	...
	result.VectorCoverage = vectorCoverage
	if vectorCoverage < 1 {
		return result, nil
	}
	...
	if err := tx.Raw(query, arguments...).Scan(&rows).Error; err != nil {
		return ucidomain.SemanticStoreResult{}, fmt.Errorf("uci projection select hybrid candidates: %w", err)
	}
	result.Candidates, err = uciHybridCandidatesFromRows(rows, ref, spec)
	if err != nil {
		return ucidomain.SemanticStoreResult{}, err
	}
	return result, nil
...
func uciHybridMetadataResult(metadata uciQueryViewMetadata, found bool, spec ucidomain.QuerySpec) (ucidomain.SemanticStoreResult, bool) {
	if !found || metadata.State == UCIViewRetired || metadata.State == UCIViewStaging {
		return uciHybridUnavailableResult(ucidomain.QueryErrorBuildIncomplete), false
	}
	...
	if spec.Mode == ucidomain.QueryModeFTS {
		...
		if !lexicalOK || lexical == ucidomain.IndexCoverageUnavailable {
			return uciHybridUnavailableResult(ucidomain.QueryErrorParserUnsupported), false
		}
	...
	return ucidomain.SemanticStoreResult{Coverage: coverage}, true
}

func uciHybridCandidatesFromRows(rows []uciHybridQueryCandidateRow, ref ucidomain.ContextRef, spec ucidomain.QuerySpec) ([]ucidomain.SemanticCandidate, error) {
	candidates := make([]ucidomain.SemanticCandidate, 0, len(rows))
	...
	for _, row := range rows {
		...
		if !ok {
			return nil, fmt.Errorf("uci projection select hybrid candidates: invalid page candidate")
		}
		...
		candidates = append(candidates, ucidomain.SemanticCandidate{Candidate: candidate, MatchSources: sources})
	}
	...
	return `
		...
		scoped_candidates AS (
			...
			LEFT JOIN ci_definitions AS definition
				ON definition.artifact_id = chunk.artifact_id
				AND definition.local_symbol_key = chunk.symbol_key
			WHERE ` + strings.Join(conditions, uciHybridConditionSeparator) + `
		)
`, arguments, nil
}
...
func buildUCIHybridCandidatesSQL(ref ucidomain.ContextRef, profile ucidomain.VectorProfile, vector []float32, spec ucidomain.QuerySpec) (string, []any, error) {
	...
	return `
		...
		lexical_candidates AS (
			...
			FROM scoped_candidates AS candidate
			WHERE ` + strings.Join(lexicalConditions, uciHybridConditionSeparator) + `
		),
...
func uciHybridCandidateColumns(alias string) string {
	return strings.Join([]string{
		...
		alias + uciHybridRelativePathColumn,
		alias + uciHybridByteStartColumn,
		alias + uciHybridByteEndColumn,
		alias + ".chunk_kind",
		alias + ".language",
		alias + ".chunk_id",
...
func uciHybridIdentityJoin(left, right string) string {
	return strings.Join([]string{
		...
		left + uciHybridRelativePathColumn + " = " + right + uciHybridRelativePathColumn,
		left + uciHybridByteStartColumn + " = " + right + uciHybridByteStartColumn,
		left + uciHybridByteEndColumn + " = " + right + uciHybridByteEndColumn,
	}, uciHybridConditionSeparator)
}

func uciHybridTieOrder(alias string) string {
	return strings.Join([]string{
		alias + uciHybridRelativePathColumn + ` COLLATE "C" ASC`,
		alias + uciHybridByteStartColumn + " ASC",
		alias + uciHybridEntityKeyColumn + ` COLLATE "C" ASC`,
		alias + uciHybridArtifactIDColumn + " ASC",
		alias + uciHybridChunkContentDigestColumn + ` COLLATE "C" ASC`,
		alias + uciHybridByteEndColumn + " ASC",
	}, ", ")
}
...
func digestUCIBytes(value []byte) string {
	...
	return uciProjectionDigestPrefix + hex.EncodeToString(sum[:])
}
...
func (s *UCIProjectionStore) loadMutableUCIArtifact(ctx context.Context, artifactID string) (*UCIParseArtifact, error) {
	...
	if err := s.db.WithContext(ctx).Where(uciProjectionArtifactIDWhere, artifactID).First(&artifact).Error; err != nil {
...
func (s *UCIProjectionStore) DescribeIndexArtifact(ctx context.Context, sourceID, artifactID string) (ucidomain.IndexArtifactProof, error) {
	...
	var artifact UCIParseArtifact
	if err := s.db.WithContext(ctx).Where(uciProjectionSourceArtifactWhere, sourceID, artifactID).First(&artifact).Error; err != nil {
	...
	factsDigest := uciProjectionDigestPrefix + hex.EncodeToString(hash.Sum(nil))
...
func (s *UCIProjectionStore) hashUCIDefinitions(ctx context.Context, hash interface{ Write([]byte) (int, error) }, artifactID string, byteLength int64) (uint64, error) {
	...
	rows, err := s.db.WithContext(ctx).Where(uciProjectionArtifactIDWhere, artifactID).Order("local_symbol_key ASC, definition_id ASC").Model(&UCIDefinition{}).Rows()
	if err != nil {
...
func (s *UCIProjectionStore) hashUCIReferenceSites(ctx context.Context, hash interface{ Write([]byte) (int, error) }, artifactID string, byteLength int64) (uint64, error) {
	...
	rows, err := s.db.WithContext(ctx).Where(uciProjectionArtifactIDWhere, artifactID).Order("site_key ASC, reference_site_id ASC").Model(&UCIReferenceSite{}).Rows()
	if err != nil {
...
func (s *UCIProjectionStore) hashUCIChunks(ctx context.Context, hash interface{ Write([]byte) (int, error) }, artifactID string, blob UCIBlob) (uint64, error) {
	...
	rows, err := s.db.WithContext(ctx).Where(uciProjectionArtifactIDWhere, artifactID).Order("ordinal ASC, chunk_id ASC").Model(&UCIChunk{}).Rows()
	if err != nil {
...
func loadUCICurrentViewForCheckout(ctx context.Context, tx *gorm.DB, checkout *UCICheckout) (*UCIView, error) {
	...
	var view UCIView
	if err := tx.WithContext(ctx).Where(uciProjectionViewIDWhere, *checkout.CurrentViewID).First(&view).Error; err != nil {
...
func loadUCIPublishedViewForJob(ctx context.Context, tx *gorm.DB, job UCIJob) (ucidomain.IndexPublishedView, error) {
	...
	var view UCIView
	if err := tx.WithContext(ctx).Where(uciProjectionViewIDWhere, *job.ResultViewID).First(&view).Error; err != nil {
...
func canonicalUCIPublicationDigest(kind string, value any) (string, error) {
	...
	return uciProjectionDigestPrefix + hex.EncodeToString(sum[:]), nil
}
...
func lockUCIPublicationJobByID(ctx context.Context, tx *gorm.DB, buildID string) (*UCIJob, error) {
	var job UCIJob
	if err := tx.WithContext(ctx).Clauses(clause.Locking{Strength: "UPDATE"}).Where(uciProjectionJobIDWhere, buildID).First(&job).Error; err != nil {
		return nil, errUCIPublicationRejected
	}
	return &job, nil
}
...
func (publisher *uciPublisher) validateArtifactProfile(ctx context.Context, sourceID, artifactID, profileID string) error {
	...
	if err := publisher.store.db.WithContext(ctx).Where(uciProjectionSourceArtifactWhere, sourceID, artifactID).First(&row).Error; err != nil {
		return errUCIPublicationRejected
	}
	var profile UCIAnalysisProfile
	if err := publisher.store.db.WithContext(ctx).Where(uciProjectionProfileIDWhere, profileID).First(&profile).Error; err != nil {
		return errUCIPublicationRejected
	}
...
func loadUCIPublicationParts(ctx context.Context, db *gorm.DB, buildID string) ([]ucidomain.IndexPart, []ucidomain.IndexPartAck, error) {
	...
	if err := db.WithContext(ctx).Where(uciProjectionBuildIDWhere, buildID).Order("sequence ASC").Find(&rows).Error; err != nil {
...
func validateUCIPublicationCandidateShape(candidate *uciPublicationCandidate, mode ucidomain.IndexManifestMode) error {
	for sourcePath, replacement := range candidate.EdgeReplacements {
		if err := validateUCIPublicationCandidateReplacementShape(candidate, mode, sourcePath, replacement); err != nil {
			return err
		}
	}
	...
	return validateUCIPublicationFullCandidateShape(candidate)
}

func validateUCIPublicationCandidateReplacementShape(candidate *uciPublicationCandidate, mode ucidomain.IndexManifestMode, sourcePath string, replacement ucidomain.IndexEdgeReplacement) error {
...
func validateUCIPublicationFullCandidateShape(candidate *uciPublicationCandidate) error {
...
func (publisher *uciPublisher) validateUCIPublicationCandidateArtifacts(ctx context.Context, scope ucidomain.IndexScope, profileID string, candidate *uciPublicationCandidate) error {
	...
	return publisher.validateUCIPublicationCandidateArtifactsForProfile(ctx, scope, profile, candidate)
}

func (publisher *uciPublisher) validateUCIPublicationCandidateArtifactsForProfile(ctx context.Context, scope ucidomain.IndexScope, profile UCIAnalysisProfile, candidate *uciPublicationCandidate) error {
...
func validateUCIPublicationCandidateEdges(candidate *uciPublicationCandidate) error {
	...
	for sourcePath, replacement := range candidate.EdgeReplacements {
		for _, edge := range replacement.Edges {
			if err := validateUCIPublicationCandidateEdge(candidate, seenEdgeKeys, sourcePath, edge); err != nil {
				return err
			}
		}
	}
...
func validateUCIPublicationCandidateEdge(candidate *uciPublicationCandidate, seenEdgeKeys map[string]struct{}, sourcePath string, edge ucidomain.IndexEdge) error {
...
func (publisher *uciPublisher) sealUCIPublicationArtifacts(ctx context.Context, tx *gorm.DB, sourceID string, proofs map[string]ucidomain.IndexArtifactProof, now time.Time) error {
	...
	for _, artifactID := range artifactIDs {
		...
		var artifact UCIParseArtifact
		if err := tx.WithContext(ctx).Clauses(clause.Locking{Strength: "UPDATE"}).Where(uciProjectionSourceArtifactWhere, sourceID, artifactID).First(&artifact).Error; err != nil {
			return errUCIPublicationRejected
		}
