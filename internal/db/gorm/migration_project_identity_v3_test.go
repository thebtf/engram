...
func TestProjectIdentityV3Migration162(t *testing.T) {
	...
	for _, table := range []string{"project_identifiers", "project_merge_audits"} {
		var count int
		require.NoError(t, db.Raw(`
	...
	for index, predicate := range map[string]string{
		"idx_projects_project_key":       "where (project_key is not null)",
	...
	audit := ProjectMergeAudit{
		SourceProjectKeys: pq.StringArray{sourceKey},
	...
	var identifierCount, auditCount int64
