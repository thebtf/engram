...
// its content carries relative-time language AND it is older than window.
//
// IMPORTANT: pass the memory's created_at, NOT updated_at. updated_at is a row-mutation
// timestamp (bumped by injection/citation/lifecycle writes without touching content),
// so an injected "currently…" memory would have a perpetually-fresh updated_at and never
// flag. created_at is immutable: its worst case is over-warning an edited memory, which
// is the safe direction for a re-verify hint. See rationale.go for the full reasoning.
// window <= 0 falls back to DefaultFreshnessWindow.
//
