...
func processBoundaryTask(prompt, mode string) *loomlib.Task {
	env := map[string]string{}
	if mode != "" {
		env["LOOM_HELPER_MODE"] = mode
	}
	return &loomlib.Task{
		ID:     "process-boundary-task",
...
func TestCliWorker_ProductionReadinessStructuredArgsAndCWD(t *testing.T) {
	...
	cwd := t.TempDir()
	task := processBoundaryTask("structured prompt", "state")
	task.Role = "maker"
	...
	require.NoError(t, err)
	childDir, err := os.Stat(state.CWD)
	require.NoError(t, err)
	assert.True(t, os.SameFile(requestedDir, childDir), "child cwd %q must identify requested cwd %q", state.CWD, cwd)
...
func TestCliWorker_ProductionReadinessOutputLimit(t *testing.T) {
	...
	w := loomhandler.NewCLIWorkerWithAllowlist([]string{loomCLIHelperName})
	result, err := w.Execute(context.Background(), processBoundaryTask("", "huge"))
	require.NoError(t, err)
...
func TestCliWorker_ProductionReadinessOutputLimitUnalignedWrite(t *testing.T) {
	...
	w := loomhandler.NewCLIWorkerWithAllowlist([]string{loomCLIHelperName})
	result, err := w.Execute(context.Background(), processBoundaryTask("", "huge-unaligned"))
...
func TestCliWorker_ProductionReadinessExitWithoutStderr(t *testing.T) {
	...
	w := loomhandler.NewCLIWorkerWithAllowlist([]string{loomCLIHelperName})
	_, err := w.Execute(context.Background(), processBoundaryTask("", "exit"))
	require.Error(t, err)
