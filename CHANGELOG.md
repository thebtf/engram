...
- Fixed OMP Engram MCP startup from an ordinary project directory: npm and marketplace plugin configurations now resolve the package wrapper, while Codex retains its own package-relative configuration (#531).

## [6.49.3] - 2026-09-16

### Fixed

- Corrected the combined artifact fixture to include the required Linux `engram-server` stub.
...
- Synchronized the generated bootstrap release policy with the 6.49.1 package manifests.
...
- Fixed the PostgreSQL image build.
...
- **Fail-closed OpenClaw publication (#428, #430).** The npm workflow now runs the complete plugin test suite and refuses publication unless the local stable semantic version is strictly newer than the current registry version, preventing changed source from silently shipping under an already-published version.
