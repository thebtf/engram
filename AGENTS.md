...
For ordinary manual browser/consumer verification, prefer the provided Chrome DevTools or browser-harness tools connected to the existing dedicated CDP browser on port 9222. Playwright is only for an existing automated suite when specifically needed; missing checkout Playwright or `node_modules` alone does not block ordinary browser verification. Do not launch or install a duplicate browser. Product-server availability is a separate prerequisite.
...
`docs/RELEASE-PROTOCOL.md` is the single operational source for commands, CI compute limits, release authority and rollback. Attempt exact-candidate Sonar when available. If its service, tool or runtime is unavailable, record `SKIPPED_UNAVAILABLE / NOT_PROVEN` with a concrete diagnostic and continue other checks and delivery; do not repair Sonar unless separately requested. This never waives actual analyzed code/security findings or other mandatory security, supported-platform and migration gates. The v6.49.4 PR #531 exception remains historical evidence, not the availability boundary. Routine already-authorized actions need no renewed approval.
