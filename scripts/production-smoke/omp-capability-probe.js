 ...
 *
 * HAP-01C has one project authority: the tracked `repository` anchor at the
 * scratch Git root. The OMP turn runs from that same root so the extension and
 * plugin-managed child present one anchor ID with one scope to the server.
 * Reusing the ID under a nested `directory` scope would correctly require an
 * operator binding decision and invalidate the qualification.
 */
function writeScenarioProjectAnchor(runtime, directory, deps, scope) {
...
async function runOmpTurn(runtime, scenarioRoot, plugin, directCredentials, deps) {
 ...
 const workspace = runtime.options.scratch_dir;
