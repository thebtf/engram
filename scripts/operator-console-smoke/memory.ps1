...
Assert-Contains $memory "v-for=`"action in actionGaps`"" "data-backed detail action rendering"
Assert-Contains $memory "action.labelKey" "descriptor-backed action label"
Assert-Contains $memory "action.badgeKey" "descriptor-backed action badge"
Assert-Contains $memory "action.evidence.endpoint" "descriptor-backed action evidence"
Assert-Contains $memory "utilityKnown" "unknown utility guard"
Assert-Contains $memory "confidenceKnown" "unknown confidence guard"
Assert-Contains $memory "usePersistentPageSize(MEMORY_PAGE_SIZE_STORAGE_KEY, 10)" "persistent memory page size"
...
Assert-Contains $memoryComposable "MEMORY_LIST_LIMIT = 500" "bounded all-mode memory list cap"
Assert-Contains $memoryComposable '/api/memories?project=${encodeURIComponent(project)}&limit=${MEMORY_LIST_LIMIT}' "project-scoped memory endpoint"
Assert-Contains $memoryComposable "mapStatus(row)" "status-based row classification"
Assert-Contains $memoryComposable "utilityKnown" "usage-counter presence mapping"
Assert-Contains $memoryComposable "confidenceKnown" "confidence presence mapping"
...
Assert-Contains $memoryComposable "export interface MemoryActionGap extends OperatorUnsupportedAction" "reusable memory action descriptor type"
Assert-Contains $memoryComposable "export const memoryActionGaps" "exported memory action descriptor data"
Assert-Contains $memoryComposable "memory.detail.actions.hideNoise" "design-contract hide-noise action"
Assert-Contains $memoryComposable "memory.detail.actions.promote" "design-contract lifecycle action"
Assert-Contains $memoryComposable "overview.badges.mustBuild" "honest unsupported action badge"
