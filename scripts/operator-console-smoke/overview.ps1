...
Assert-Contains $overview "modelsError" "live model-health error binding"
Assert-Contains $overview "queueGated" "honest review-queue gate binding"
Assert-Contains $overview "accessGap" "honest access gap binding"
Assert-Contains $overview "graphFlagBadge" "graph flag-derived badge binding"
...
Assert-Contains $overviewComposable "useModelsState" "live model health seam"
Assert-Contains $overviewComposable "'access-summary'" "access mustbuild descriptor"
Assert-Contains $overviewComposable "operator-overview-graph-flag" "graph flag endpoint evidence"
Assert-Contains $overviewComposable "ENGRAM_GRAPH_ENABLED" "strict graph flag gate"
Assert-Contains $overviewComposable "graphFlagState.value = 'error'" "graph flag error state"
...
Assert-NotContains $overview "meta: [{ cls: 'live', text: t('common.enabled') }]" "unconditional graph enabled badge"
