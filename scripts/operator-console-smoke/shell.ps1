...
Assert-Contains $layout "useOperatorMemoryLab" "statusbar memory live-state seam"
...
Assert-Contains $layout "memoryLab.pending.value && memories.length === 0" "statusbar must not show false zero while memory is pending"
Assert-Contains $layout "t('shell.recordsLoading')" "statusbar keyed loading records label"
Assert-NotContains $layout "t('shell.records', memoryCount)" "statusbar raw memory count while pending"
