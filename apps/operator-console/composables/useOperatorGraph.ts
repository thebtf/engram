import { onScopeDispose, type ComputedRef, type Ref } from 'vue'
...
export function useOperatorGraph(): {
  ...
  const refreshGeneration = useState<number>('live:graph:refresh-generation', () => 0)
  const edgesGeneration = useState<number>('live:graph:edges-generation', () => 0)
  const traverseGeneration = useState<number>('live:graph:traverse-generation', () => 0)
  const pathGeneration = useState<number>('live:graph:path-generation', () => 0)
  const mutationGeneration = useState<number>('live:graph:mutation-generation', () => 0)
  ...
  async function refreshConnectedEdges(nodeID = selectedNodeID.value) {
    ...
    if (nodesStateRef.value.kind === 'gated' || nodesStateRef.value.kind === 'pending') return
