...
test('refresh invalidates a delayed traverse response', async ({ page }) => {
  ...
  await page.goto('/graph')
  const runTraverse = page.locator('#graph-traverse-memory').locator('xpath=ancestor::article').getByRole('button')
  await page.locator('#graph-traverse-memory').fill('memory-one')
