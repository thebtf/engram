...
async function fillLegacyImport(page: Page) {
 await page.getByLabel('Ссылка на источник').fill('truth-state.md')
 await page.getByLabel('Содержимое источника').fill('# Truth state\n\nPlaintext only.')
}

test('Books states name legacy-only capability, unsupported input, and the next safe action', async ({ page }) => {
 await page.goto('/books')
 const books = page.locator('.books-page')
 const statebar = books.locator('.statebar')

 await expect(books.locator('.page-head .hb')).toHaveAttribute('data-state', 'empty')
 await expect(books.locator('.page-head .hb')).not.toContainText('работает')
 await expect(books.locator('.books-brief')).toContainText('не Book Context')
 await expect(statebar).toHaveAttribute('data-state', 'empty')
 await expect(page.getByRole('status', { name: 'Нет активного legacy-импорта' })).toContainText('Далее:')
 await expect(page.getByLabel('Текстовый файл-источник')).toBeVisible()
 await expect(page.getByLabel('Ссылка на источник')).toBeVisible()
 await expect(page.getByLabel('Содержимое источника')).toBeVisible()

 await page.getByLabel('Текстовый файл-источник').setInputFiles({
  name: 'unsupported.pdf',
  mimeType: 'application/pdf',
  buffer: Buffer.from('%PDF-1.7'),
 })
 await expect(statebar).toHaveAttribute('data-state', 'unsupported')
 await expect(page.getByRole('status', { name: 'Этот формат источника здесь не поддержан' })).toContainText('Далее:')
})

test('Books separates denied, error, timeout, and offline outcomes from an empty import queue', async ({ page }) => {
 const books = page.locator('.books-page')
 const statebar = books.locator('.statebar')

 for (const scenario of [
  { status: 401, state: 'denied', title: 'Доступ к legacy-импорту запрещён' },
  { status: 500, state: 'error', title: 'Сервер не подтвердил импорт' },
  { status: 408, state: 'timeout', title: 'Время запроса импорта истекло' },
 ] as const) {
  await page.route('**/api/books', jsonRoute({ error: scenario.state }, scenario.status))
  await page.goto('/books')
  await fillLegacyImport(page)
  await page.getByRole('button', { name: 'Поставить legacy-импорт' }).click()
  await expect(statebar).toHaveAttribute('data-state', scenario.state)
  await expect(page.getByRole('alert', { name: scenario.title })).toContainText('Далее:')
  await page.unroute('**/api/books')
 }

 await page.route('**/api/books', (route) => route.abort('connectionrefused'))
 await page.goto('/books')
 await fillLegacyImport(page)
 await page.getByRole('button', { name: 'Поставить legacy-импорт' }).click()
 await expect(statebar).toHaveAttribute('data-state', 'offline')
 await expect(page.getByRole('alert', { name: 'Сервер недоступен' })).toContainText('Далее:')
})

test('Books reports loading, partial legacy output, and stale readback with keyboard recovery', async ({ page }) => {
 let releaseCreate: (() => void) | undefined
 const created = new Promise<void>((resolve) => { releaseCreate = resolve })
 await page.route('**/api/books', async (route) => {
  await created
  await route.fulfill({ contentType: 'application/json', body: JSON.stringify({ id: 8, status: 'pending', source_ref: 'truth-state.md' }) })
 })
 await page.route('**/api/books/8/status', jsonRoute({
  id: 8,
  status: 'done',
  source_ref: 'truth-state.md',
  documents_path_prefix: 'books/jobs/8/',
 }))

 await page.goto('/books')
 await fillLegacyImport(page)
 const statebar = page.locator('.books-page .statebar')
 await page.getByRole('button', { name: 'Поставить legacy-импорт' }).click()
 await expect(statebar).toHaveAttribute('data-state', 'loading')
 await expect(page.getByRole('status', { name: 'Проверяется legacy-импорт' })).toContainText('Далее:')

 releaseCreate?.()
 await expect(statebar).toHaveAttribute('data-state', 'partial')
 await expect(page.getByRole('status', { name: 'Созданы только legacy Documents' })).toContainText('Book Context')
 await expect(page.getByRole('link', { name: 'Открыть созданные Documents' })).toBeVisible()

 await page.unroute('**/api/books/8/status')
 await page.route('**/api/books/8/status', jsonRoute({ error: 'unavailable' }, 500))
 const refresh = statebar.getByRole('button', { name: 'Обновить статус' })
 await refresh.focus()
 await expect(refresh).toBeFocused()
 await page.keyboard.press('Enter')
 await expect(statebar).toHaveAttribute('data-state', 'stale')
 await expect(page.getByRole('status', { name: 'Доступен только прежний результат импорта' })).toContainText('Далее:')
