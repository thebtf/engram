<template>
  <nav class="fixed top-0 left-0 right-0 z-50 bg-gradient-to-b from-slate-950 via-slate-950/95 to-transparent backdrop-blur-md">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 py-4 sm:py-5 flex justify-between items-center">
      ...
      <ul class="hidden md:flex gap-6 lg:gap-8">
        <li v-for="link in links" :key="link.href">
          <a
            :href="link.href"
            class="text-slate-400 hover:text-white text-sm font-medium transition-colors animated-underline"
          >
            {{ link.label }}
          </a>
        </li>
      ...
      <!-- Actions: GitHub link + CTA + mobile hamburger -->
      <div class="flex items-center gap-3 sm:gap-4">
        <a
          :href="githubUrl"
          target="_blank"
          rel="noopener noreferrer"
          class="text-slate-400 hover:text-white text-lg sm:text-xl transition-colors"
        ...
        <a
          href="#installation"
          class="hidden sm:inline-flex bg-gradient-to-br from-amber-500 to-amber-400 text-slate-950 px-4 sm:px-5 py-2 sm:py-2.5 rounded-lg font-semibold text-sm hover:scale-105 hover:shadow-lg hover:shadow-amber-500/30 transition-all"
        >
          Get Started
        </a>
        <button
    ...
    >
      <div
        v-if="mobileMenuOpen"
        class="md:hidden bg-slate-900/95 backdrop-blur-xl border-t border-slate-800 px-4 sm:px-6 py-4"
      >
        <a
          v-for="link in links"
          :key="link.href"
