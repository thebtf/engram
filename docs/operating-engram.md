...
A healthy v6.49.3 daemon status does not prove that an ordinary OMP MCP session connected or advertised usable tools; verify that separately with a fresh session's `tools/list` below.

### Prepare the server Vault and retire only an owned legacy daemon

Before enabling the Workspace chooser, confirm that the server loads its existing persistent Vault key. In a replicated deployment, every replica must use the same key; do not rely on separate auto-generated `vault.key` files. Use the supported `ENGRAM_VAULT_KEY` or `ENGRAM_ENCRYPTION_KEY_FILE` configuration and verify the non-secret Vault fingerprint across replicas without exposing key material. The operator Code HTTP adapter fails closed if the Vault key is unavailable. Back up and retain the key under the deployment's normal secret-handling procedure; replacing it also affects existing Vault credentials.
...
Before ordinary first use, inspect the selected working copy's `.engram-project` and its existing project resolution. An OMP session, a project V2 selector or V3 anchor, and a UCI Source/Checkout are three different identities: opening a fresh session or registering a code checkout does not migrate historical project-scoped memories and issues. The primary Engram checkout observed here has the legacy name-only `{"name":"engram"}` marker; the candidate worktree has a V3 anchor. V3 strictly rejects the name-only marker, and V2 selector-only resolution can be ambiguous. If either condition applies, **stop project-scoped operations**; do not force-register a fresh UUID project row, copy an anchor from another worktree, edit the marker to appear V3, or infer authority from a Git remote or checkout label.

Migration of existing project history requires owner-approved, authenticated-admin work on the **same existing project row**: an audited UUID backfill across each affected data family, with before/after counts and privacy/authorization proof. Migration 162 does not backfill old project rows or text-scoped memories and issues; V3 registration creates a new row rather than adopting that history. A normal workstation keycard cannot authorize this migration, and an `auth_disabled` synthetic admin is not an authenticated actor. The installed v6.49.3 and PR #508 do not supply this migration contract or its authorization. Do not perform manual SQL/UUID repairs, disable auth, or treat this guide as migration approval; resume the historical project path only after the owner approves a specified migration, a real admin identity is available, and its per-family preservation and access checks pass. The separate UCI first-use steps below apply only to an authorized installed release and eligible checkout; they do not clear this project-history stop.

1. From the supported origin, sign in as the intended human source owner. Confirm that this browser session has a persisted principal and that the workstation keycard you will use belongs to that **same principal**, with read-write source access. In an authorized admin session, issue the keycard at `<server-origin>/access` using **Workstation keycards**; `/tokens` is not a console page. Keep `ENGRAM_AUTH_ADMIN_TOKEN` on the server host. Configure the local client with its own keycard using the supported plugin setup. The universal plugin config is `~/.engram/config.json` (`server_url` and `api_token`); some hosts use `ENGRAM_CONFIG_FILE` or launcher overrides. Inspect the active launcher and preserve existing settings instead of replacing the file. `ENGRAM_URL` is the bare server origin for env-based setups, not an HTTP `/mcp` endpoint. Never paste real credentials into diagnostics or this document.
2. Check the server's effective `ENGRAM_CODE_INTEL_ENABLED` via `/api/flags` and separately inspect the **actual local daemon's** inherited flag. Current source defaults code intelligence on unless the value is exactly `false`; setting `true` is not an ordinary prerequisite. An older installed server or daemon may still require `true`, so use its advertised tools and effective configuration rather than assuming it follows current source. A server-side flag does not register local tools. For parser-required JS, TS, and TSX indexing, the daemon needs an installed absolute `ENGRAM_UCI_PARSER_EXECUTABLE` paired with `ENGRAM_UCI_PARSER_BUNDLE_DIGEST`. Read the compatible bundle digest from the parser's own installed output or release metadata, not the executable's file SHA-256. The installer must supply and validate the binary and configuration. The checked plugin-bin installation had no parser; this is a delivery gap, not a request for the user to guess an executable path. Inspect the daemon's own startup configuration without exposing values of secrets; a new shell's environment does not prove what the running daemon inherited. Reconnect or restart only that daemon through the supported plugin procedure.
...
4. In an installed release containing first-use onboarding, the human source owner calls MCP `codebase_context` with `{"action":"register","source_label":"my-repository","locator":"file:///absolute/local/worktree"}` using that read-write keycard and the same persisted principal as the browser user. The daemon must own that local Git working copy; registration returns server-issued `source_id`, `checkout_id`, `analysis_profile_id`, and a no-View `context_handle`, with `binding_kind:"checkout"` and `context:null`. If the response is lost or the client reconnects, the same owner on the same workstation can repeat registration with the same label and locator **only when the existing checkout has a durable registration profile binding**; a retry with its returned source ID and original locator has the same requirement. A legacy checkout with no `registration_profile_id` returns `REGISTRATION_PROFILE_UNBOUND` for either retry: the original issued profile ID is not recoverable from a View, intent, or timestamp, even if exactly one View exists. Do not modify or delete the old checkout, create a second Source with the same label, or guess an ID. Authorized published Views remain selectable via `codebase_context` `list`. For new indexing, register the same local working copy under a **different human-friendly source label**; this issues a distinct Source, checkout, and profile identity, not recovery of the old identity or historical history. Restoring the original identity requires independently proven original ID and a separately authorized procedure, which this ordinary client flow does not provide. For a second working copy under a bound Source, use `{"action":"register","source_id":"<returned source_id>","locator":"file:///absolute/local/other-worktree"}`. Keep each checkout's returned handle separate. Do not send the local paths to the browser. Registration does not grant browser reads; grant each checkout separately in [Open and accept Workspace](#open-and-accept-workspace). A newly registered checkout has no published View and will not appear in `codebase_context` `list` until indexed. If the installed daemon does not advertise `register` or the grant flow is unavailable, first indexing and F1 remain blocked; never insert database rows or invent identifiers.
...
From the supported browser origin, sign in as the same persisted human owner used for registration. Open **Home → Workspace → Share code access**. Select the first owned working copy, select your own enabled browser account as the reader, and choose **Allow reading**. Repeat for the second working copy; a grant for one checkout does not cover the other. The chooser obtains owner `choice_ref` and reader `target_ref` from `GET /api/code/grants/choices` and submits them to `POST /api/code/grants`; the server checks ownership and the target account. Display labels are not authorization. A distinct enabled reader is optional and needs a separate grant for each checkout it should read. The owner then opens **Home → Workspace → Repository → Working copy → Indexed snapshot** in a separate browser tab. The Workspace tab performs its own handshake, `POST /api/code/contexts`, and selects the returned server-issued choice. For a first View, the selected no-View checkout yields `index_intent_selection_ref` for the UI's `POST /api/code/index-intents`; poll the returned intent until the released View appears, then select it. Confirm search, off-page graph neighbor, and source evidence without typing a binding, UUID, or direct hidden URL. Browser grants belong to the selected authenticated reader and source or checkout owner; the workstation keycard alone cannot grant browser access. HTTPS or a secure browser origin does not grant read authority by itself. Do not use insecure-browser flags or a synthetic admin as a substitute for an origin and identity decision. On the inspected HTTP LAN installation, `crypto.randomUUID` was unavailable before the handshake; recheck the actual chosen supported origin after rollout.

Chooser references are AES-GCM sealed for the authenticated browser session with a key derived from the server Vault. They expire after 15 minutes. If a chooser submission is rejected after a delay or Vault key rotation, reload the choices and submit the newly returned references; do not reuse a saved `choice_ref`, `target_ref`, or `selection_ref`. During rotation, keep replicas on one key version before issuing new choices; outstanding references sealed under the old key become invalid. A refreshed reference still requires the normal owner, reader, and tab authorization checks.
...
4. **F4: graph and evidence.** Follow a direct and a reverse relation to an off-page neighbor. Select the relation, inspect its evidence precision and, when a reference-site descriptor is available, read that exact byte span and content digest through the pinned View's source action; this is the call/reference site, not the destination definition. Read the off-page neighbor separately and continue from it. Partial, ambiguous, unsupported-language, or unavailable reference-site evidence has no exact-site action; report the limit rather than claiming source proof. Do not create nodes or edges manually.
...
| Owned working copy or self reader missing from **Share code access** | Check that `/api/auth/me` shows a persisted human browser principal, not `auth_disabled` synthetic identity. Confirm that the read-write keycard used for both registrations belongs to that exact principal and that the browser account is enabled. `GET /api/code/grants/choices` lists only checkouts owned by that principal and enabled reader targets; no SQL or token inspection is needed. |
