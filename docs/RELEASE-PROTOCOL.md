...
| Bootstrap client release assets | Existing release steps invoke `prepare-bootstrap-policy.sh --check`, `check-bootstrap-policy-artifacts.sh`, `readback-bootstrap-policy-assets.sh`, and `verify-bootstrap-release-assets.js` to verify the committed client target policy against built raw launcher bytes, the policy in every release archive, and the draft's uploaded client asset sizes and digests before publication | committed policy differs from built clients, an archive carries different policy bytes, or the private draft lacks a matching uploaded client asset |
