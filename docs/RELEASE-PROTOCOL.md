...
| SonarQube Quality Gate | A designated QA subagent attempts `node tools/quality/run-sonarqube.mjs` locally from the clean, frozen exact candidate when the service, tool and runtime are available; retain `.agent/e/sonarqube/<HEAD>.json` on success or concrete unavailability diagnostics; no GitHub Actions workflow performs this gate | an actual analyzed code/security finding remains blocking, or an available analysis has a non-OK Quality Gate; unavailable service/tool/runtime is `SKIPPED_UNAVAILABLE / NOT_PROVEN`, not a release blocker or PASS |
| Bootstrap client and parser release assets | Existing release steps invoke `prepare-bootstrap-policy.sh --check`, `check-bootstrap-policy-artifacts.sh`, `readback-bootstrap-policy-assets.sh`, and `verify-bootstrap-release-assets.js` to verify committed client targets against built raw launcher bytes, policy in release archives, and uploaded client asset sizes and digests; Workspace parser checks additionally enforce the generated parser policy, raw parser asset, every package archive, and the draft's uploaded parser digest before publication | client or parser policy differs from built assets or source, archive bytes differ, or private draft lacks matching pinned assets |
...
| SonarQube gate | Attempt exact-candidate local analysis when available; record exact-head receipt or concrete unavailability diagnostics | Unavailability does not hold release: continue other checks and delivery without Sonar repair unless separately requested. Actual analyzed findings and all other gates retain their normal disposition requirements. |
...
### Historical v6.49.4 Sonar exception (2026-09-25)
...
This one-time authorization governed the v6.49.4 PR #531 campaign only; its recorded `UNAVAILABLE / NOT_PROVEN` status, diagnostics and unresolved findings remain historical truth. It neither clears findings nor waives other release, security, image, review, migration or rollout gates. The current availability rule above applies to all candidates; the historical candidate-scope limits do not govern later availability decisions.
...
For plugin acceptance in local ordinary-host smoke, record whether client bytes were already cached. Successful `initialize`, `tools/list`, and recall with a warm cache prove that client path, not a cache-miss download. For bootstrap or client-policy changes that affect first start, exercise a cache miss in the existing focused consumer smoke and verify download compatibility with host/bootstrap deadlines. Recovery using cached, exact verified release bytes does not establish a shipped downloader fix.
...
A designated QA subagent attempts `node tools/quality/run-sonarqube.mjs` from the clean, frozen exact candidate when the service, tool and runtime are available. Root accepts the evidence and directs the next delivery action; no GitHub Actions workflow performs this gate. Successful runs retain the exact-head receipt at `.agent/e/sonarqube/<HEAD>.json`. If service, tool or runtime is unavailable, retain the concrete diagnostic, label Sonar `SKIPPED_UNAVAILABLE / NOT_PROVEN`, and continue other checks and delivery without repairing Sonar unless separately requested. An actual analyzed finding is not unavailability.
...
When recovery modes are unavailable, do not guess flags or their semantics. Inspect the ordinary failure and retained diagnostics, including command, working directory, candidate HEAD/tree, redacted output, exit status and any `.scannerwork/report-task.txt` or `.agent/e/sonarqube/<HEAD>.json`. Missing or failed `--help` alone is not a reason to start the full default campaign. An unavailable service, tool or runtime does not require diagnosis, recovery or repeat campaigns to proceed with other gates. Do not fabricate completion or a recovery mode.

An advertised recovery mode is never a bypass: an available run must retain fresh exact-candidate analysis and Quality Gate `OK`, all mandatory coverage, security, review, migration and rollback obligations, and exact evidence. It must not skip required tests, alter thresholds or analysis scope, suppress actual findings, or fabricate an exact-head PASS.

## Terminal Verdict

- `PROJECT_RELEASE_PROTOCOL_PASS`: all mandatory gates have evidence; available Sonar analysis has its exact-head receipt and finding disposition, or service/tool/runtime unavailability is recorded as `SKIPPED_UNAVAILABLE / NOT_PROVEN` with a concrete diagnostic. Never report unavailable Sonar as PASS or waive actual analyzed findings or another gate.
- `PROJECT_RELEASE_PROTOCOL_BLOCKED`: a mandatory non-Sonar gate is missing, stale, failed or cannot be verified, or an actual analyzed Sonar finding/available non-OK Quality Gate remains blocking. Sonar unavailability alone is not a blocker.
